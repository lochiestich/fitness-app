...
`abductors` has its own traced body-map region, carved out of what used to be part of `glutes` — Abductor is the only exercise that trains it, so it's the only thing that lights it up. `adductors` has no traced region of its own: the `quads` region in this trace is only the inner-thigh slivers with no separate main-mass shape to fall back on, so `adductors` shares that region with `quads` instead, averaged the same way the body map already blends `shoulders` (`front_delts` + `side_delts`). Training Adductor colours the same area a leg press or squat does, blended. Volume, fatigue numbers and the Body screen's score list still track all four (`glutes`, `abductors`, `quads`, `adductors`) as fully independent muscles regardless — only the map's visual region is shared for the quads/adductors pair. Both browse under "Legs".
