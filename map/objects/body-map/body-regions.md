...
This replaced an earlier version built from plain `<rect>` shapes (14 abstract rounded rectangles, one per region) with real vector contours traced from a reference anatomy image, run through connected-component segmentation and manually classified into the app's 18 `MuscleId`s. The rectangle version was cheap to build but didn't look like a body; the trace is expensive data (15 `REGIONS` entries, ~144 path strings, ~110KB) but is real anatomy. The source trace and the Python pipeline that produced this file are **not checked into this repo** — they live in a separate scratchpad workspace kept deliberately outside the app per the project's own working style ("separate to the app for the time being"). This file is the finished export; regenerating it requires that external pipeline, so treat the `REGIONS` array as opaque, hand-maintained data, not something to hand-edit path-by-path. Exception: a region's top-level `muscles: MuscleId[]` array (as opposed to its `front`/`back` path data) is safe to hand-edit directly in this file — that's how `abductors`/`adductors` were added, with no new tracing at all.

`abductors` shares the whole `glutes` shape, and `adductors` shares the whole `quads` shape — both confirmed against a reference diagram the user provided, where each is shown covering the full extent of the muscle it rides along on, not a sub-region carved out of it. Both blend the same way `shoulders` merges `front_delts` + `side_delts`: a squat and an adductor machine both light up the same quads-shaped area; a hip thrust and an abductor machine both light up the same glutes-shaped area.
...
- 12 of the 15 regions map 1:1 to a `MuscleId`; **three carry more than one**, all averaged the same way by `BodyMap.tsx`: `shoulders` merges `front_delts` + `side_delts` per SPEC.md §6 ("Shoulders is shown as one region combining front/side delts"); `glutes` also carries `abductors`, and `quads` also carries `adductors` (SPEC.md §3) — neither has a traced shape of its own, both ride along on the existing glutes/quads region so a hip thrust and an abductor machine (or a squat and an adductor machine) light up the same area, blended. `rear_delts` is its own separate region. See the Shoulders collision in `../../CONTEXT.md`.
...
- **looks-like-but-is-not:** a per-`MuscleId` shape list — it's per-*region*, and `shoulders`, `glutes` and `quads` each cover two `MuscleId`s
