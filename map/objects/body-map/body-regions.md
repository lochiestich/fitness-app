...
- 12 of the 15 regions map 1:1 to a `MuscleId`; **three carry more than one**, all averaged the same way by `BodyMap.tsx`: `shoulders` merges `front_delts` + `side_delts` per SPEC.md §6 ("Shoulders is shown as one region combining front/side delts"); `glutes` also carries `abductors`, and `quads` also carries `adductors` (SPEC.md §3) — those two have no traced shape of their own, they ride along on the existing glute/quad region so a squat and an abductor machine both light up the same area, blended. `rear_delts` is its own separate region. See the Shoulders collision in `../../CONTEXT.md`.
